...
npm run server     # 启动本地后端 http://localhost:8787（跨设备分享 / AI 生成图纸需要）
```

> **后端说明**：`server/index.mjs` 是零依赖 Node 服务，提供「跨设备分享」存取与「AI 文生图」接口（读取根目录 `.env` 的 `DASHSCOPE_API_KEY`，文生图模型可用 `WANX_MODEL` 配置，默认 `wanx2.1-t2i-turbo`）。开发时 Vite 已把 `/api` 代理到 8787；部署时可把该服务部署到任意 Node 主机/VPS，并把前端请求指向后端地址。
