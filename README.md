...
| 🖼️ 图片转图纸 | 上传任意图片 → **可选上传前裁剪**（拖拽选区/手柄缩放）+ 默认宽度按图片自适应 + 边缘锐化 + 色彩增强 + 细节档位（超采样 1/2/3 倍）+ **背景处理（默认背景留空：从边缘扩散只去掉外围背景、图案内部保留；默认自动裁剪空白边距）** + 最近色 / Floyd-Steinberg 抖动量化 → 生成图纸（默认显示色号，预览缩放流畅）；生成后可 **🎨 颜色优化**：合并相似色 / 去杂色（少于 N 颗并入最近色）/ 逐色排除与重映射（留空、自动最近色、替换为其他色号，实时预览） |
| 🧮 图纸详情 | 网格预览（可开关色号/网格、调格子大小）、用豆统计、缺豆提醒 |
...
node scripts/deep-test.cjs  # 上传图片→生成→颜色优化→裁剪→A4打印→库存→编辑(撤销/重做)→导入→持久化 全流程测试
